...
impl ExampleWrapper {
    ...
    pub fn process_event(&mut self) -> Result<(), IBKRApiLibError> {
        match self.client.get_event()? {
            Some(ServerRspMsg::NextValidId  { version, order_id }) =>
            {
            ...
            Some(ServerRspMsg::ErrMsg { version, req_id, error_code, error_str }) => self.error(req_id, error_code, &error_str),
            Some(ServerRspMsg::TickPrice { version, req_id, tick_type, price, tick_attr }) =>
                info!("tick_size -- req_id: {}, tick_type: {}, price: {}, attrib: {}", req_id, tick_type, price, tick_attr),
            Some(ServerRspMsg::TickSize { version, req_id, tick_type, size }) =>
            ...
            Some(ServerRspMsg::MarketDataType { version, req_id, market_data_type}) =>
                info!("market_data_type -- req_id: {}, market_data_type: {}", req_id, market_data_type),
            Some(ServerRspMsg::ManagedAccts { version, accounts_list }) =>
            ...
            Some(ServerRspMsg::ExecutionDataEnd { version, req_id }) => info!("exec_details_end -- req_id: {}", req_id),
            Some(ServerRspMsg::CommissionReport { version, commission_report }) =>
